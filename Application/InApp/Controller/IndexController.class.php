...
class IndexController extends Controller {
    ...
    public function index(){
    	...
    	// 支付宝和微信回调 
    	// 主要区别 一个用POST直接接收 一个用file_get_contents("php://input")接收 
    	// 支付宝回调数据 数组格式返回 微信回调数据 xml格式返回
        ...
        $this->renderSuccess('测试成功',array('status' => 'OK'));
    }
		...
		if($alipayNotify->getResponse($_POST['notify_id']))//判断成功之后使用getResponse方法判断是否是支付宝发来的异步通知。
		{
			if(!$alipayNotify->getSignVeryfy($_POST, $_POST['sign'])) {//使用支付宝公钥验签
				...
				//请务必判断请求时的out_trade_no、total_fee、seller_id与通知时获取的out_trade_no、total_fee、seller_id为一致的
					...
					$orderStatus = $this->checkOrderStatus($notifyData);
					// 如果订单不是已完成订单 则更新相关数据
					if(!$orderStatus)
					{
						// $this->updateCourseOrderData($notifyData);
						$this->updateOrderData($notifyData);
					}
	...
	private function parseOutTradeNo($out_trade_no = 'MTMjMSMxOCMxNDkwMjUwMzEw')
	{
		...
		// pay_type "Rechange" 和 "Course"
		$pay_type = strtolower($outTradeNoDataTemp['4']);

		$outTradeNoData = array(
								'id'			=>	$outTradeNoDataTemp['0'],
								'uid'			=>	$outTradeNoDataTemp['1'],
								$pay_type.'_id'	=>	$outTradeNoDataTemp['2'],
								'create_time'	=>	$outTradeNoDataTemp['3'],
								'pay_type'		=>	$outTradeNoDataTemp['4'],
							);
	...
	// 更新课程订单状态和与此订单相关的信息
	private function updateCourseOrderData($notifyData,$outTradeNoData)
	{
		...
		// var_dump('updateCourseOrderDataInner');
	...
	// 更新充值订单状态和与此订单相关的信息
	private function updateRechangeOrderData($notifyData,$outTradeNoData)
	{
		$updateOrderData = array(
									'trade_no'		=>	$notifyData['trade_no'],
									'pay_time'		=>	$notifyData['notify_time'],
		...
		$whereUpdateOrderData = array(
										'id'			=>	$outTradeNoData['id'],
										'uid'			=>	$outTradeNoData['uid'],
										'rechange_id'	=>	$outTradeNoData['rechange_id'],
										'create_time'	=>	$outTradeNoData['create_time'],
										'rechange_price'=>	$notifyData['total_fee']
								);
		// var_dump('updateRechangeOrderDataInner');
		$OrderRechange = M('OrderRechange');

		$OrderRechange->where($whereUpdateOrderData)->save($updateOrderData);
	...
	// 更新订单状态和与此订单相关的信息
	private function updateOrderData($notifyData)
	{
		...
		$pay_type = array_pop($outTradeNoData);

		$payUpdateOrderDataName = 'update'.$pay_type.'OrderData';
		// var_dump('updateOrderDataInner');

		$this->$payUpdateOrderDataName($notifyData,$outTradeNoData);

	}

	// 检查订单状态
	private function checkOrderStatus($notifyData)
	{
		// out_trade_no 由notifyData中获取
		// $out_trade_no = 'MTMjMSMxOCMxNDkwMjUwMzEw';
		$out_trade_no = $notifyData['out_trade_no'];
		...
		$OrderObj = M('Order'.$outTradeNoData['pay_type']);

		$pay_type = strtolower($outTradeNoData['pay_type']);

		$whereUpdateOrderData = array(
										'id'			=>	$outTradeNoData['id'],
										'uid'			=>	$outTradeNoData['uid'],
										$pay_type.'_id'	=>	$outTradeNoData[$pay_type.'_id'],
										'create_time'	=>	$outTradeNoData['create_time']
								);

		$orderStatus = $OrderObj->where($whereUpdateOrderData)->getField('status');
