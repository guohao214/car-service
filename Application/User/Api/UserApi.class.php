...
class UserApi extends Api{
    ...
    public function batchMemberInfo($uids) {
        $info = array();
        $Member = M('member');
        $info = $Member->field('uid,nickname,avatar,signature')->where('uid in ('.$uids.')')->select();
        if(is_array($info)) {
            //默认头像
            foreach ($info as &$row) {
     ...
     * @param string $uids 用户id字符串
     */
    public function starMemberInfo($uids) {
        $info = array();
        $Member = M('member_org');
            $info = $Member->alias('a')
                    ->field('a.uid,a.nickname,m.avatar,m.signature')
                    ->join('__MEMBER__ m ON m.uid=a.uid')
                    ->where('a.uid in ('.$uids.')' )
                    ->group('a.uid')
                    ->select();
     ...
     * @param string $uids 用户id字符串
     */
    public function starMemberInfoo($uids,$org_id) {
        $info = array();
        $Member = M('member_org');
            $info = $Member->alias('a')
                    ->field('a.uid,a.nickname,m.avatar,m.signature')
                    ->join('__MEMBER__ m ON m.uid=a.uid')
                    ->where(array('a.uid in ('.$uids.')' , 'a.org_id'=>$org_id ) )
                    ->group('a.uid')
                    ->select();
     ...
     * @param string $uids 用户id字符串
     */
    public function classMemberInfo($uids,$org_id) {
        $info = array();
    ...
    public function sendMessage($uid, $type, $extra_info) {
        ...
        if(M('message')->add($data)) {
            $this->sendNotice($uid, $title, $data['content']); //发送通知
        }
    }
    ...
    public function sendNotice($uid, $title, $content) {
        $push_id = $this->getPushId($uid);
var_dump($push_id);exit;
        if(!$push_id) {
            return false;
        }
        vendor('Getui/Getui');
    ...
    private function getPushId($uid) {
        ...
        $map['id'] = $uid;
        $info = $Model->field('push_id')->where($map)->find();
        if(!empty($info['push_id'])) {
            return $info['push_id'];
        }
        return false;
    }
