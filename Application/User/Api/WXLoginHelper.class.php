...
class WXLoginHelper {
    ...
    public function checkLogin($code, $rawData, $signature, $encryptedData, $iv) {
        ...
        //$pc = new WXBizDataCrypt('wx2c6383b19f336b63', $sessionKey);
    ...
    public function decryptData( $encryptedData, $iv,$sessionKey)
    {
        if (strlen($sessionKey) != 24) {
            return json_encode('sessionKey错误');
        }
        $aesKey=base64_decode($sessionKey);


        if (strlen($iv) != 24) {
            return json_encode('iv错误');
        }
        ...
        // if( $dataObj  == NULL )
        // {
        //     return json_encode('IllegalBuffer错误');
        // }
    ...
    public function makeRequestJson($url, $params_json, $expire = 0, $extend = array(), $hostIp = '')
    {
        if (empty($url)) {
            return array('code' => '100');
        }
        ...
        // 只要第二个参数传了值之后，就是POST的
        curl_setopt($_curl, CURLOPT_POSTFIELDS, $params_json);
        curl_setopt($_curl, CURLOPT_POST, true);
