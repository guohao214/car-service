...
class MemberModel extends Model{
    ...
    public function autoLogin($user){
        ...
        //2017 04-07 sufyan
        $gift_coin = M('gift_exchange')->field('total_coins')->where('uid='.$user['uid'].' and (type=0 or type=2)')->order('add_time desc')->find();
        if(empty($gift_coin)){
            $total_coins = 0;
        }else{
            $total_coins = $gift_coin['total_coins'];
        }
        $auth = array(
            'uid'             => $user['uid'],
            ...
            'anchor_status'        => $user['anchor_status'],
            'works'    => $user['works'],
            'likes'    => $user['likes'],
