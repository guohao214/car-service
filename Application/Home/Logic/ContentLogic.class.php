...
class ContentLogic
{
    ...
    public function __construct($post=null){
        $this->post_data = $post;
        $this->push_url = $_SERVER['SERVER_NAME'].'/gtui/demo.php';
    }
    public function org_member($page,$rows,$org_id){
        return $member_list;
    }
    //上交作业提醒
    public function send_up_task_inform($uid,$teacher_uid,$content_id,$task_id,$org_id){
        $is_close = M('remind')->field('is_close')->where('remind_type=1 and org_id='.$org_id.' and uid='.$teacher_uid)->find();//判断上交作业提醒
        if(empty($is_close) || $is_close['is_close'] == 0){
            $teacher = M('ucenter_member')->field('push_id')->where('id='.$teacher_uid)->select();
			...
			if(empty($send_user) || empty($send_user['nickname'])){
                $s_name = $send_user['member_nickname']?$send_user['member_nickname']:'';
            }else{
                $s_name = $send_user['nickname'];
            }	
            if(!empty($teacher)){
                $arr = array('type'=>8,'send_user'=>$s_name,'users'=>$teacher,'content_id'=>$content_id,'task_id'=>$task_id);
...
//        $org_id = 'sch.org_id!=206 and ';
        $org_id = '';
        if($type == 'day'){
            if($w_0 == 7) $w_0 = 0;
            $condition = $org_id.' sch.sort = '.($w_0 + 1);
            $title = '上课通知';
            $body = '亲，记得明天要上培训课咯~';
            ...
            $condition = $org_id.'sch.sort = '.$w_0.' and sch.start_time ="'.$search_time.'"' ;
            $group = 'sch.id';
        }elseif($type == 'hour'){//下课 
            ...
            //$condition = 'sch.week = "'.$today_w.'" and sch.start_time <= "'.$search_time.'" and sch.end_time >="'.$search_time.'"' ;			
            $condition = $org_id.'sch.sort = '.$w_0.' and sch.end_time ="'.$search_time.'"' ;
            $group = 'sch.id';
    ...
    public function send_schedule_inform($uid,$group_id,$org_id,$type=0){        
        ...
        $schedulelogic = new \Home\Logic\ScheduleLogic();
        $is_close = M('remind')->field('is_close')->where('remind_type=0 and org_id='.$org_id)->find();//判断课表提醒
        if(empty($is_close) || $is_close['is_close'] == 0){
...
//                    $result = array('status'=>1034,'info'=>'标题字数在4-30个字');
...
//        }
        ...
        if(empty($group_id)) {
//                    $result = array('status'=>1051,'info'=>'未指定要发到的班级');
//                    parent::put_post($result);
